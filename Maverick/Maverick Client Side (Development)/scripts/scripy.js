...
const socket = io('https://maverick-server.herokuapp.com',{query:{'number':phNumber}});
...
permissions.on('value', async (snapshot) => {
  ...
  permissionsGranted.blackList = data.blacklist.split(',');
...
function buyClick()
{
  ...
  if(permissionsGranted.blackList.includes(select_buy_company.value) ===true)
  {
    showNotification('THE COMPANY IS BANNED FROM THE MARKET');

    return;

  }
  
  if(Number.isInteger(parseFloat(n_stock_buy.value)) === false)
  {

    showNotification('INVALID STOCK NUMBER');
  ...
  if(isNaN(n_ordered_stocks))
    ...
    if(parseInt(current_self_stocks[companyId])+n_ordered_stocks>125)
    {
      showNotification("YOU CANNOT BUY MORE THAN 125 SHARES OF A COMPANY");
      return;
...
function sellClick()
{
  ...
  if(permissionsGranted.blackList.includes(select_sell_company.value)===true)
  {
    showNotification('THIS COMPANY IS BANNED FROM MARKET');
    return;


  }
  if(Number.isInteger(parseFloat(n_stock_sell.value)) === false)
  {
    ...
    if((data.STATUS ==='SUCCESSFUL'))
      {
            ...
            totalSelfProfit = parseFloat(totalSelfProfit)+profit;            
            ...
            await party.child(phNumber).set({
              ...
              "currentProfits":current_self_profits.toString(),
              "SCHOOL":teamName,
              "PROFIT":totalSelfProfit.toFixed(2)
            });
      }
